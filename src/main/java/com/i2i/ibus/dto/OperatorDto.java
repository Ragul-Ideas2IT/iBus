...
public class OperatorDto {
    ...
    private int id;
    @Pattern(regexp = "^([a-zA-Z][ ]?){2,}$", message = "Travel's name should be in valid format")
    @NotBlank(message = "Travel's name is mandatory")
    private String travelsName;
    @Pattern(regexp = "^([a-zA-Z][ ]?){2,}$", message = "Owner's name should be in valid format")
    ...
    @Pattern(regexp = "^[A-Z]{5}[0-9]{4}[A-Z]$", message = "PAN number should be in valid format")
    @NotBlank(message = "PAN number is mandatory")
    private String panNumber;
    @Pattern(regexp = "^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][0-9](Z)[0-9]$", message = "GST number should be in valid format")
